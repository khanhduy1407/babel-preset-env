...
function buildPreset(context) {
  ...
  if (debug && !hasBeenLogged) {
    hasBeenLogged = true;
    console.log("@nkduy/babel-preset-env: `DEBUG` option");
    console.log("\nUsing targets:");
  ...
  // @nkduy/babel-plugin-transform-es2015-modules-commonjs gets its spec mode
  modulePlugin && plugins.push([require("@nkduy/babel-plugin-" + modulePlugin), { loose: loose }]);

  plugins.push.apply(plugins, transformations.map(function (pluginName) {
    return [require("@nkduy/babel-plugin-" + pluginName), { spec: spec, loose: loose }];
  }));
